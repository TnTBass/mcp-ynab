...
class DeltaSyncManager:
    ...
    async def should_sync(self, plan_id: str, endpoint: str) -> bool:
        async with get_session() as session:
            stmt = select(ServerKnowledge).where(
                ServerKnowledge.plan_id == plan_id,
    ...
    async def get_knowledge(self, plan_id: str, endpoint: str) -> int | None:
        async with get_session() as session:
            stmt = select(ServerKnowledge).where(
                ServerKnowledge.plan_id == plan_id,
                ServerKnowledge.endpoint == endpoint,
    ...
    async def update_knowledge(self, plan_id: str, endpoint: str, knowledge: int) -> None:
        async with get_session() as session:
            stmt = select(ServerKnowledge).where(
                ServerKnowledge.plan_id == plan_id,
                ServerKnowledge.endpoint == endpoint,
    ...
    async def upsert_entities(
        ...
        plan_id: str,
        entity_type: str,
        entities: Sequence[YNABBaseModel],
    ) -> None:
        if not entities:
            return
        async with get_session() as session:
            for entity in entities:
                ...
                if row:
                    ...
                    session.add(CachedEntity.from_model(plan_id, entity_type, entity))
            await session.commit()
    ...
    async def has_cached_data(self, plan_id: str, endpoint: str) -> bool:
        knowledge = await self.get_knowledge(plan_id, endpoint)
        return knowledge is not None

    async def invalidate_knowledge(self, plan_id: str, endpoint: str) -> None:
        async with get_session() as session:
            stmt = select(ServerKnowledge).where(
                ServerKnowledge.plan_id == plan_id,
                ServerKnowledge.endpoint == endpoint,
